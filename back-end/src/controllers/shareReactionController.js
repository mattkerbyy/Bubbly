...
// Valid reaction types (must match post reactions)
...
export const addShareReaction = async (req, res) => {
  try {
    ...
    if (existingReaction) {
      ...
      // Update the notification to reflect the new reaction type
      ...
      // Create notification if reacting to someone else's share
      if (share.userId !== userId) {
...
export const getShareReactions = async (req, res) => {
  try {
    ...
    const groupedReactions = reactionsWithFollowStatus.reduce((acc, reaction) => {
