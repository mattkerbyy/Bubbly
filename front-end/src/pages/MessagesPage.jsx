...
export default function MessagesPage() {
  ...
  // Get otherUserId or conversationId from navigation state
  const otherUserId = location.state?.otherUserId
  const stateConversationId = location.state?.conversationId

  // Fetch or create conversation if coming from profile
  const { data: conversationData, isSuccess } = useGetOrCreateConversation(otherUserId)

  // Auto-open conversation when coming from profile or global chat
  useEffect(() => {
  ...
  useEffect(() => {
    const handleUserStatus = ({ userId, isOnline }) => {
      // Update activeOtherUser if their status changed
