...
import { useCreatePost } from "@/hooks/PostFeature/usePosts";
...
export default function CreatePost() {
  ...
  const handleFileSelect = (e) => {
    ...
    selectedFiles.forEach((selectedFile) => {
      if (selectedFile.size > 50 * 1024 * 1024) {
        alert(
          `File "${selectedFile.name}" is too large. Maximum size is 50MB per file.`
        );
        return;
      }
  ...
  const isSubmitDisabled =
    (!content.trim() && files.length === 0) || createPostMutation.isPending;
