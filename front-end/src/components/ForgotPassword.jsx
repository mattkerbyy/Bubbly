...
import { Eye, EyeOff, CheckCircle2, XCircle, Loader2, RotateCw } from 'lucide-react'
...
const ForgotPassword = () => {
  ...
  const handleRequestReset = async (e) => {
    ...
    try {
      ...
      setErrors({ submit: error.response?.data?.error || 'Failed to send reset code. Please try again.' })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      if (error.response?.status === 429) {
        toast.error(error.response?.data?.error || 'Please wait before requesting a new code.')
      } else {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      // Use toast for server errors (like invalid/expired token)
        ...
        <form onSubmit={handleResetPassword} className="space-y-5" noValidate>
            ...
            <div className="flex gap-2 justify-center">
              {otp.map((digit, index) => (
                <Input
                  ...
                  disabled={isLoading}
                  className={`w-12 h-14 text-center text-2xl font-semibold ${errors.otp ? 'border-destructive' : ''}`}
                />
              ))}
            </div>
            {errors.otp && (
          ...
          <div className="space-y-2">
            ...
            {confirmPassword && !errors.confirmPassword && (
              <div className={`p-3 rounded-lg bg-muted/50 border border-border flex items-center gap-2 text-xs ${passwordsMatch ? 'text-green-600' : 'text-destructive'}`}>
                {passwordsMatch ? (
                  <>
