...
export default function NotificationItem({ notification, onClose }) {
  ...
  const getNotificationIcon = () => {
    ...
    // Handle reaction notifications with specific icons (using emoji for consistency)
  ...
  const getNotificationText = () => {
    ...
    // Handle reaction notifications with specific text
    if (type === 'reaction' || type === 'like') {
      const reactionText = reactionType 
        ? reactionType.charAt(0).toUpperCase() + reactionType.slice(1).toLowerCase()
        : 'Like'
      
      return (
        <>
          <span className="font-semibold">{senderName}</span> reacted {reactionText} to your post
        </>
      )
    }
  ...
  const handleClick = () => {
    ...
    if (notification.type === 'follow') {
      ...
      // If it's a share notification, navigate to the share detail page
      navigate(`/share/${notification.shareId}`)
    } else if (notification.postId) {
      // For reactions and comments, navigate to the specific post
