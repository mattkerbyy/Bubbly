...
import EditShareModal from "@/components/EditFeature/EditShareModal";
...
import {
  useAddShareReaction,
  useRemoveShareReaction,
} from "@/hooks/ShareFeature/useShareReactions";
import { useShareComments } from "@/hooks/ShareFeature/useShareComments";
import { useDeleteShare } from "@/hooks/PostFeature/usePosts";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function SharePreviewModal({ isOpen, share, onClose }) {
  ...
  const [userReaction, setUserReaction] = useState(
    getUserReaction(resolvedShare)
  );
  const [reactionCount, setReactionCount] = useState(
    resolvedShare?._count?.reactions || 0
  );
  const [commentCount, setCommentCount] = useState(
    resolvedShare?._count?.comments || 0
  );
  const [shareCount, setShareCount] = useState(
    resolvedShare?.post?._count?.shares || 0
  );
  ...
  const { data: commentsData, isLoading: commentsLoading } = useShareComments(
    isOpen && shareId ? shareId : undefined
  );
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span>
                      {resolvedShare?.createdAt
                        ? formatDistanceToNow(
                            new Date(resolvedShare.createdAt),
                            {
                              addSuffix: true,
                            }
                          )
                ...
                <>
                  ...
                  <div className="flex items-center justify-around p-2">
                    <ReactionPicker
                      ...
                      disabled={
                        addShareReactionMutation.isPending ||
                        removeShareReactionMutation.isPending
                      }
                      isShare
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will remove this shared post from your timeline. The
                  original content will stay up.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
