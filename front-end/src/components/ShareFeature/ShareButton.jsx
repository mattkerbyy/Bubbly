...
import { useSharePost } from "@/hooks/ShareFeature/useShares";
...
const ShareButton = ({ post, className, showCount = true }) => {
  ...
  const getFileType = (filename) => {
    ...
    if (["jpg", "jpeg", "png", "gif", "webp", "bmp", "svg"].includes(ext)) {
      ...
      ["mp4", "mov", "avi", "wmv", "webm", "mkv", "flv"].includes(ext)
    ) {
      return "video";
    } else if (["mp3", "wav", "ogg", "flac", "aac", "m4a"].includes(ext)) {
      return "audio";
    } else if (
      ["doc", "docx", "pdf", "txt", "ppt", "pptx", "xls", "xlsx"].includes(ext)
    ) {
      return "document";
    }
    return "file";
    ...
    if (uniqueTypes.length === 1) {
      ...
      return (
        <div className="mt-2 text-xs text-muted-foreground">
          {getFileIcon(type)} {post.files.length}{" "}
          {getFileLabel(type, post.files.length)}
        </div>
      );
    }
    return (
