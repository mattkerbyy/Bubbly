...
import { REACTION_TYPES, REACTION_EMOJIS } from '@/services/reactionService'
...
// Define the order of reactions for tabs
const REACTION_ORDER = ['Like', 'Heart', 'Laughing', 'Wow', 'Sad', 'Angry']

export default function ReactionsModal({ postId, shareId, isOpen, onClose, isShare = false }) {
  ...
  useEffect(() => {
    if (activeTab === 'All') {
      ...
      setFilteredReactions(reactions.filter(r => r.reactionType === activeTab))
    }
  }, [activeTab, reactions])
  ...
  // Get sorted reactions that exist
  const availableReactions = REACTION_ORDER.filter(type => reactionCounts[type] > 0)
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {isLoading ? (
            ...
            <div className="text-center py-12 px-4 text-muted-foreground">
              <p className="text-lg mb-1">{activeTab === 'All' ? '😊' : REACTION_EMOJIS[activeTab]}</p>
              <p>{activeTab === 'All' ? 'No reactions yet' : `No ${activeTab} reactions`}</p>
            </div>
          ) : (
            <div className="p-2">
                  ...
                  >
                    ...
                    >
                      <div className="relative">
                        ...
                        <div className="absolute -bottom-1 -right-1 h-5 w-5 bg-background rounded-full flex items-center justify-center border-2 border-background shadow-sm">
                          <span className="text-base leading-none">{REACTION_EMOJIS[reaction.reactionType]}</span>
                        </div>
                      </div>
                    ...
                    {!isOwnProfile && (
                      <Button
                        ...
                        size="sm"
                        onClick={() => handleFollowToggle(reaction.user.id, isFollowing)}
                        disabled={followMutation.isPending || unfollowMutation.isPending}
                        className={cn(
                          'ml-2 group transition-all duration-200',
                          isFollowing && 'hover:bg-destructive hover:text-white hover:border-destructive'
                        )}
                      >
                        {isFollowing ? (
                          <>
