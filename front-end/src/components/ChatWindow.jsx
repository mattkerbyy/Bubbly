...
export default function ChatWindow({ conversationId, otherUser, onShowChatList }) {
  ...
  const groupMessagesByTime = (messages) => {
    ...
    messages.forEach((msg, index) => {
      ...
      // Check if we need a new time separator (every 5+ minutes)
      const shouldShowTime = 
        index === 0 || 
        differenceInMinutes(msgTime, new Date(messages[index - 1].createdAt)) >= 5
  ...
  // Mark messages as read when new unread messages from other user appear
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (newMessage) => {
      // New message received for this conversation - update cache immediately
  ...
  queryClient.invalidateQueries({ queryKey: ['conversations'] })
    ...
    const handleUserStoppedTyping = ({ conversationId: typingConvId, userId }) => {
      if (typingConvId === conversationId && userId === otherUser.id) {
        setIsTyping(false)
      }
    }
    ...
    const handleMessagesRead = ({ conversationId: readConvId }) => {
      if (readConvId === conversationId) {
        // Immediately update messages to mark them as read
        queryClient.setQueryData(['messages', conversationId], (oldData) => {
          if (!oldData) return oldData
    ...
    // Cleanup - pass callbacks to remove only these specific listeners
    return () => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      >
        ...
        >
          ...
          <p className="text-xs text-muted-foreground">
            {(otherUser.isOnline ?? otherUser.isActive) ? 'Active now' : 'Offline'}
          </p>
        </div>
      </motion.div>
      ...
      <div 
        className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4 bg-accent/30"
      >
          ...
          {messageGroups.map((group, groupIndex) => (
            <div key={groupIndex} className="space-y-1">
              ...
              {group.messages.map((msg, msgIndex) => {
                ...
                const showAvatar = msgIndex === 0 || group.messages[msgIndex - 1].senderId !== msg.senderId
                const nextMessage = group.messages[msgIndex + 1]
                const showTimestamp = !nextMessage || nextMessage.senderId !== msg.senderId
                  ...
                  >
                    ...
                    {showAvatar ? (
                      <Avatar 
                        ...
                        onClick={() => handleAvatarClick(isSent ? currentUser.username : otherUser.username)}
                      >
                        <AvatarImage
                          src={getImageUrl(isSent ? currentUser.avatar : otherUser.avatar)}
                          alt={isSent ? currentUser.name : otherUser.name}
                        />
                        <AvatarFallback>
                          {(isSent ? currentUser.name : otherUser.name).charAt(0).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
                    ) : (
                    ...
                    <div className={`flex flex-col ${isSent ? 'items-end' : 'items-start'} max-w-[70%]`}>
                      ...
                      {isLastMessage && isSent && (
                        <span className="text-xs text-muted-foreground mt-1 px-2">
                          {msg.isRead 
                            ? `Seen ${formatDistanceToNow(new Date(msg.createdAt), { addSuffix: true })}`
                            : `Sent ${formatDistanceToNow(new Date(msg.createdAt), { addSuffix: true })}`
                          }
                        </span>
                      )}
                    </div>
