...
export const useFollowUser = () => {
  ...
  return useMutation({
    ...
    onMutate: async (userId) => {
      ...
      // Optimistically update profile follower count
      if (previousProfile) {
...
export const useInfiniteFollowers = (userId) => {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) => followService.getFollowers(userId, pageParam),
...
export const useInfiniteFollowing = (userId) => {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1 }) => followService.getFollowing(userId, pageParam),
    getNextPageParam: (lastPage) => {
      const { currentPage, hasMore } = lastPage.pagination
      return hasMore ? currentPage + 1 : undefined
    },
